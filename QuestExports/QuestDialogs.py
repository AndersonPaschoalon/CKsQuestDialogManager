...
class QuestDialogs:
    ...
    def generate_documentation(self, destination, app_name: str, github_url: str):
        """
        ...
        if self.quest_name.strip() == "":
            _log.warn("**WARN** quest name is empty. Skipping quest documentation generation.")
            return
        # (1) Initialize environment
        _log.debug("generate_documentation() destination:" + destination + " for quest " + self.quest_name)
            ...
            if not branch.is_branch_data_empty():
                ...
                for topic in branch.list_topic_dialogs:
                    if not topic.is_topic_data_empty():
                        ...
                        _log.warning("** TOPIC DATA IS EMPTY FOR TOPIC <" + topic.topic_name + "> **")
                        _log.warning("** TOPIC <" + topic.topic_name +
                                          "> IS NOT GOING TO BE ADDED TO THE DOCUMENTATION  **")
            else:
                _log.warning("** BRANCH DATA IS EMPTY FOR BRANCH <" + branch.branch_name + "> **")
                _log.warning("** BRANCH <" + branch.branch_name +
                                  "> IS NOT GOING TO BE ADDED TO THE DOCUMENTATION  **")
        ...
        if not is_empty:
            ...
            for scene in self.list_scenes:
                if not scene.is_empty():
                    ...
                    _log.warn("**WARN** SKIPPING EMPTY SCENE.")
        else:
            _log.info("!! THERE IS NO SCENE FOR QUEST <" + self.quest_name + "> !!")
        ...
        md += "> _Documentation generated by **{0}**. Access the project <a href=\"{1}\" target=\"_blank\">Github</a> for new versions and updates._\n\n"\
            .format(app_name, github_url)
        _log.debug("markdown generated -> saving into files")
        # (4) Generating documentation
        # Markdown
        _log.debug("creating markdown")
        md_file = open(path_md, "w")
        md_file.write(md)
        md_file.close()
        # html
        test_gen_bootstrap = True
        _log.debug("creating html")
        if test_gen_bootstrap:
            Md.html_file(md, path_html, self.quest_name)
        else:
    ...
    @staticmethod
    def generate_quest_documentation(skyrim_path: str, comment_csv: str, actors_csv: str, scene_order_csv: str,
                                     doc_dir: str, app_name: str, github_url: str):
        """
        ...
        _log.debug("generate_quest_documentation() skyrim_path:" + skyrim_path + ", comment_csv:" +
                   comment_csv + ", actors_csv:" + actors_csv + ", doc_dir:" + doc_dir + ", app_name:" + app_name +
