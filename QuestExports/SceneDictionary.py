...
class SceneDictionary:
    ...
    SUPPORTING_DIALOGUE = 'SUPPORTING DIALOGUE'
    EMOTION = 'EMOTION'
        ...
        with Cd(skyrim_path):
            for nth_file in export_dialog_files:
                ...
                with open(nth_file) as fd:
                    ...
                    first_row = next(rd)
                    col_scene = SceneDictionary._get_index(first_row, SceneDictionary.SCENE)
                    col_alias = SceneDictionary._get_index(first_row, SceneDictionary.ALIAS)
                    col_voice = SceneDictionary._get_index(first_row, SceneDictionary.VOICE_TYPE)
                    col_file = SceneDictionary._get_index(first_row, SceneDictionary.FILENAME)
                    col_response = SceneDictionary._get_index(first_row, SceneDictionary.RESPONSE_TEXT)
                    col_supporting = SceneDictionary._get_index(first_row, SceneDictionary.SUPPORTING_DIALOGUE)
                    col_emotion = SceneDictionary._get_index(first_row, SceneDictionary.EMOTION)
                    col_notes = SceneDictionary._get_index(first_row, SceneDictionary.SCRIPT_NOTES)
                    col_phase = SceneDictionary._get_index(first_row, SceneDictionary.SCENE_PHASE)
                    for row in rd:
                        sd = SceneDictionary()
                        ...
                        if not any(x.filename == sd.filename for x in list_scenes_dics):
                            ...
                            print("Dic already have entry x.filename:" + sd.filename + " (" + nth_file + ")")
    ...
    def list_scene_files(skyrim_path: str, quest_id: str):
        ...
        for nth_file in all_files:
            if (nth_file.startswith(SceneDictionary.SCENE_PREFIX) and
                    (nth_file.endswith(SceneDictionary.SCENE_SUFIX)) and
...
if __name__ == '__main__':
    ...
    for sd in lsd:
        #print("<" + sd.dialogue + ">")
        print("<" + sd.filename + ">")
        #print("sd.to_string():" + sd.to_string());