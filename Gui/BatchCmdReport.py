...
<!-- partial -->
    ...
    # 6 - Console Output
    HTML_REPORT_FOOTER = """    <li class="table-row">
    ...
    def export_report(list_batch_cmds_reports, app_dir: str):
        ...
        filename = app.settings_obj.docgen_dir + filename + dt_string + ".html"
        html_str = BatchCmdReport.HTML_REPORT_HEADER
        item: BatchCmdReport
        for item in list_batch_cmds_reports:
            color = 'blue' if item.error_flag else 'blue'
            result = 'SUCCESS' if item.error_flag else 'ERROR'
            html_item = BatchCmdReport.HTML_REPORT_FOOTER.format(color, result, item.command, item.error_code,
                                                                 item.error_message, item.process_code, item.stdout)
            html_str += html_item
        html_str += BatchCmdReport.HTML_REPORT_FOOTER
